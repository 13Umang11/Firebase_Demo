...
const [count,setcount] = useState('')
  ...
  useEffect(() => {
    ...
    if (user) {
      messaging()
        .getToken()
        .then(token => {
          firestore().collection('UserToken').doc(`${user.uid}`).set({
            userID: user.uid,
            token: token,
            // doneAt: '',
          });
 ...
 const channelId = await notifee.createChannel({
    ...
    .then(() => {notifee.getBadgeCount();
      console.log('jbjknrlrelvnlkaren')})
  }
        ...
        <View style={{flex: 1, justifyContent: 'center'}}>
          ...
          <TouchableOpacity onPress={Timer} style={styles.btn}>
            <Text style={styles.btntext}>
              Small Image & Sub Title Notification
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity onPress={Priority} style={styles.btn}>
            <Text style={styles.btntext}>Priority Notification</Text>
          </TouchableOpacity>
          
          {Platform.OS === 'ios' ?
          <TouchableOpacity onPress={()=>{notifee.getBadgeCount()}} style={styles.btn}>
            <Text style={styles.btntext}>Bandage Notification</Text>
          </TouchableOpacity>
